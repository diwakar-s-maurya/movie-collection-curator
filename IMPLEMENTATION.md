...
- Which database that instance talks to is decided in `src/db.ts` and nowhere else: under Vitest it is `TEST_DATABASE_URL` **or an error**, never a fallback to `DATABASE_URL`. That matters because the suite truncates every table between cases, and the root `.env` the vitest config loads carries both urls into a process every worker inherits from. Two guards, because they fail differently: `db.ts` checks which *variable* it reads, and `vitest.config.ts` checks the *value* — it refuses to start if `TEST_DATABASE_URL` names the same database as `DATABASE_URL`, which is the half that goes wrong quietly, since the two urls differ by one word and a shell export beats the file.
- A transaction is `db.$transaction(async (tx) => ...)` inside the service that owns the write, using `tx` for the statements in it. What the single instance gives up is a transaction spanning several services, since none of them takes a client to hand `tx` to. Nothing planned needs that — the counters in README 100x item 2 and the tag table in decision 4 are both multi-row writes inside one service — and the escape hatch, if it is ever wanted, is an AsyncLocalStorage context in `db.ts` rather than a parameter on every function.
- `collection_id = ANY($1)` in the summary query needs an explicit cast on a uuid column (`ANY(ARRAY[...]::uuid[])` under `$queryRaw`), otherwise Postgres compares uuid to text and errors.
