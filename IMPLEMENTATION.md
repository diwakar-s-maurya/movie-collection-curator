...
One procedure, `collectionMovies.updateAnnotation`, takes a partial of `{ note, tags, rating }` and answers `ok`. It does not send the row back: the client is editing a row it already holds, and the only thing the response could add is how the tags and the note were normalised, which the client applies to its own copy with the same rules. Returning the row would cost a read of the whole snapshot on every star click, since the write itself reports only whether there was a row to write to. Rating is a one-click interaction from the card and note plus tags are saved together from the annotation dialog, but they are the same row and the same write, so they share one procedure and one mutation hook (`useUpdateAnnotation(collectionId)`) on the client. The hook patches the row optimistically with `queryClient.setQueriesData` on the partial key for that collection's `collectionMovies.list`, so every cached page is updated without knowing which page holds the row (the patch stands as the new value on success; a failure undoes its own fields and refetches the pages, so overlapping writes to one row cannot undo each other), and invalidates `collections.get` (summary and breakdowns changed) and `collections.list` (list cards show the average). Nothing refetches the movie page for a one-field change that succeeds. Gives up: a PATCH whose meaning depends on which keys are present, and a little cache-patching code that deserves a comment.
...
- `updateAnnotation`: optimistic patch of the row into every cached page for the collection with `setQueriesData` on the partial key, nothing to swap in on success; invalidate `collections.get` (summary + breakdown) and `collections.list` (list cards show the average). A failure undoes the fields that write set and refetches the pages, since a rating clicked while a note is saving overlaps it: undoing to the page as it was found would drop the write that succeeded in between, and two writes to one field need the server to say which landed.
...
Nothing refetches the whole collection for a one-field change that succeeds.
